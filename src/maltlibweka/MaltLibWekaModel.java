...
public class MaltLibWekaModel implements Serializable, MaltLibModel {
  ...
  /**
   * Construct a MaltLibWekaModel with the specified weka classifier and maximum class number.
   * (classification might return any number up to the upperbound - 1)
   * 
   * @param classifier
   * @param attinfo
   ...
   * @param nomMap
   */
  public MaltLibWekaModel(Classifier classifier, FastVector attinfoBeforeHacks,
      FastVector attinfoPostHacks, Map<String, FastVector> nomMap, int classUpperBound) {
  ...
  /**
   * We're given an array of MaltFeatureNode, and we have to come up with a classification. In order
   * to do that, produce a weka instance.
   */
  @Override
  ...
  /**
   * Create the weka Instances object that we'll use to hold our weka instances, then classify them.
   * Has to know about the appropriate attributes. We may mangle these.
   * 
   * @param x
   * @return
   */
  private Instances getInstances(MaltFeatureNode[] x) {
    ...
    save_instances.setClass(save_instances.attribute(save_instances.numAttributes() - 1));
    return this.save_instances;
  }

  private Instances classifierSpecificHacks(Instances instances) throws Exception {
   ...
   * 
   * @param features Array of MaltFeatureNode.
   * @param instances
   * @param attinfo
   * @return
   */
  private Instance featuresToInstance(MaltFeatureNode[] features, FastVector attinfo) {
  ...
  /**
   * Change the nominal attributes into binary ones, then filter out the attributes that weren't
  ...
  private Instances binarizeAndFilter(Instances nominals) throws Exception {
    ...
    for (int i = 0; i < attInfoPostHacks.size(); i++) {
      ...
      binarized.setValue(i, nominal.stringValue(field).equals(val) ? 1 : 0);
    }
