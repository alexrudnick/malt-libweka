...
/**
 * Extends MaltParser with parameterizable Weka classifiers. Extends the same class as LibLinear and
 * LibSvm, so can be used in a similar way. Just calls out to Weka classifiers instead.
 * 
 * @author Alex Rudnick
 */
public class LibWeka extends Lib {
  ...
  public LibWeka(InstanceModel owner, Integer learnerMode) throws MaltChainedException {
  ...
  public boolean predict(FeatureVector featureVector, SingleDecision decision)
    ...
    for (int i = 0; i < featureVector.size(); i++) {
      ...
      if (sfv.getValue() == 1) {
        ...
        throw new MaltChainedException("did not expect a value other than 1 or 0.");
      }
    }
  ...
  protected void trainInternal(FeatureVector featureVector) throws MaltChainedException {
    try {
      owner.getGuide().getConfiguration().getConfigLogger()
      ...
      Class<Classifier> classifierClass =
          (Class<Classifier>) owner.getGuide().getConfiguration()
      ...
      String optstring =
          owner.getGuide().getConfiguration().getOptionValueString("libweka", "wekaopts");
      ...
      ObjectOutputStream output =
          new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream(getFile(".moo")
              .getAbsolutePath())));
      try {
      ...
      e.printStackTrace();
      throw new LibException("The Weka learner was not able to redirect Standard Error stream. ", e);
    } catch (SecurityException e) {
      throw new LibException("The Weka learner cannot remove the instance file. ", e);
    } catch (IOException e) {
      throw new LibException("The Weka learner cannot save the model file '"
          + getFile(".mod").getAbsolutePath() + "'. ", e);
    } catch (Exception e) {
      e.printStackTrace();
      throw new LibException("The Weka learner broke in some other way", e);
    }
  }
  ...
  /**
   * Modifies the weka dataset based on specific knowledge we have about the classifier. This is
   * kind of terrible, but we're going to do it anyway.
   * 
   * @param instances
   * @param classifier
   * @return
   * @throws Exception
   */
  private Instances classifierSpecificHacks(Instances instances, Classifier classifier)
      throws Exception {
  ...
  private Instances simpleFilterAttributes(Instances instances, int maxAttributes) throws Exception {
    ...
    System.out.println("after filtering, num attributes: " + out.numAttributes());
    return out;
  }

  private Instances buildWekaInstances(InputStreamReader isr) throws MaltChainedException {
    try {
  ...
  /**
   * Find the highest class number in the instances file and add 1 to it, so we know how many
   * different possible classes there are for classification.
   * 
  ...
  /**
   * Build and return the map from names of features (which are just named by their column from the
   * file they're read from), to a FastVector of the possible values that they can take.
   * 
   * @return
   * @throws MaltChainedException
   */
  ...
  /**
   * Accessor for the attribute-value counts; computes them on demand if needed.
   * 
   * @return An ArrayList where each element is a Map mapping from a given possible value to the
   *         count of times it occurred in the training data. Positions correspond to the positions
   *         of the attribute.
   * @throws MaltChainedException
   */
  public ArrayList<HashMap<String, Integer>> getAttributeValueCounts() throws MaltChainedException {
  ...
  /**
   * Produce the list of maps, where each map contains the counts for each value, for the attribute
   * in the corresponding position.
   * 
   * @return
   * @throws MaltChainedException
   */
  private ArrayList<HashMap<String, Integer>> countAttributeValues() throws MaltChainedException {
